...
% torso
pMt = [-r * sin(q1) + l * sin(q1+q3); r * cos(q1) - l * cos(q1+q3)];
% stance leg
pm1 = [-(r/2) * sin(q1); (r/2) * cos(q1)];
% swing leg
pm2 = [-r*sin(q1) + (r/2) * sin(q1 + q2); r*cos(q1) - (r/2) * cos(q1 + q2)];
% center of mass
pcm = (m/(2 * m + Mh + Mt)) * pm1 + (m/(2 * m + Mh + Mt)) * pm2 + (Mh/(2 * m + Mh + Mt)) * pMh + (Mt / (2 * m + Mh + Mt)) * pMt;
% end of swing leg
P2 = [-r * sin(q1) + r * sin(q1 + q2);  r * cos(q1) - r * cos(q1 +q2)];
...
V_Mh = Mh * g * r * cos(q1); 
V_Mt = Mt * g * (r * cos(q1) - l * cos(q1 + q3));
V_m1 = m * g * (r/2) * cos(q1);
V_m2 = m * g * (r * cos(q1) - (r/2) * cos(q1 + q2));
% Total PE
...
% Vector fields
% x_dot = fx + gx * u
% where, x_dot = 6x1 matrix, u = 2x1 matrix
fx = [dq; inv(D) * (-C * dq - G)]; % 6x1 matrix
gx = [0 0 ; 0 0; 0 0 ; inv(D) * B]; % 6x2 matrix
